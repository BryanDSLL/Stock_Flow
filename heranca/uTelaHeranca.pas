...
// CRIAÇÃO DA PROCEDURE PARA CONTROLAR OS BOTÕES
...
// VERIFICAÇÃO DA QUERY E CRIAÇÃO DO FORM
...
{$region 'CONEXÃO E FUNCIONAMENTO DA GRID E PESQUISA'}

// CONEXÃO E FUNCIONAMENTO DA GRID E PESQUISA
...
// PROCEDIMENTO PARA OS BOTÕES DE CONTROLE
        ...
        if (Components[i] is TLabeledEdit) then
          begin
            if (TLabeledEdit(Components[i]).Tag = 1) and (TLabeledEdit(Components[i]).Text = EmptyStr) then
              begin
                messageDlg(TLabeledEdit(Components[i]).EditLabel.Caption + ' é um campo obrigatório!', TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0);
                Result := True;
                Break;
              end;
          end;
